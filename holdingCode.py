...
<div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 "> 
    ...
    <div class="flex items-center border-b-2 border-teal-200 mb-6">
        <div class="flex flex-wrap gap-2">
            ...
            <!-- Inside the tabs div TODAY -->
            <button id="deliveryRouteTab" class="px-4 py-2 font-semibold text-teal-700 border-b-2 border-transparent hover:border-teal-600 focus:outline-none tab-button transition duration-300 rounded-t-lg">Delivery Route</button>
        </div>
            ...
            <div class="bg-white p-6 rounded-2xl shadow-lg w-full">
                ...
                <form method="POST" id="listingForm" class="space-y-4" enctype="multipart/form-data">
                    ...
                    <div>
                        ...
                        <div class="flex space-x-2">
                            <input type="text" id="listingLocationInput" name="location" class="w-full p-2 border rounded-md focus:ring-teal-500 focus:border-teal-500" placeholder="Enter or select your location" value="{{ form.location.value|default:user.userprofile.county }}">
                            <button type="button" id="useCurrentLocationListing" class="p-2 bg-teal-700 text-white rounded-md hover:bg-teal-800 transition duration-300" title="Use Current Location">
        ...
        <div class="bg-white p-6 rounded-2xl shadow-lg w-full max-w-md">
            ...
            <form id="orderForm" class="space-y-4">
                ...
                <div>
                    ...
                    <div class="flex space-x-2">
                        <input id="orderLocationInput" type="text" name="location" class="w-full p-2 border rounded-md focus:ring-teal-500 focus:border-teal-500" placeholder="Enter or select your location" required>
                        <button type="button" id="useCurrentLocationOrder" class="p-2 bg-teal-700 text-white rounded-md hover:bg-teal-800 transition duration-300" title="Use Current Location">
                            <i class="fas fa-location-arrow"></i>
                        </button>
                    </div>
                    <input type="hidden" id="orderLatitude" name="latitude">
                    <input type="hidden" id="orderLongitude" name="longitude">
                </div>
                <div>
                    <label class="block text-gray-700">Delivery Mode</label>
                    <select id="orderDeliveryMode" name="deliveryMode" class="w-full p-2 border rounded-md focus:ring-teal-500 focus:border-teal-500" required>
...
<script>
    ...
    let currentListingId = null;
        ...
        document.getElementById('orderForm').addEventListener('submit', async (e) => {
            ...
            const locationInput = document.getElementById('orderLocationInput'); // Fixed ID the orderers location at current
            const latitudeInput = document.getElementById('orderLatitude');
            const longitudeInput = document.getElementById('orderLongitude');
            const deliveryModeInput = document.getElementById('orderDeliveryMode');

            if (!quantityInput || !locationInput || !deliveryModeInput) {
                console.error('Missing required inputs:', { quantityInput, locationInput, deliveryModeInput });
            ...
            const deliveryMode = deliveryModeInput.value;
            const listingId = orderForm.dataset.listingId || currentListingId; // Fallback to currentListingId

            console.log('Form data:', { listingId, quantity, location, latitude, longitude, deliveryMode });

            if (!listingId || isNaN(quantity) || quantity <= 0 || !location || !deliveryMode) {
       ...
       // Farmer Orders (Requested Orders))
        async function showFarmerOrders() {
            ...
            data.orders.forEach(order => {
                ...
                // Encode the default message to avoid URL issues
                const defaultMessage = encodeURIComponent(`Hello, regarding your order #${order.id} for ${order.crop}`);
                // Create WhatsApp link if phone number exists
                const whatsappLink = order.phone_number 
                ? `<a href="https://wa.me/${order.phone_number}?text=${defaultMessage}" target="_blank" class="text-green-600 hover:text-green-800"><i class="fab fa-whatsapp"></i></a>`
                : '<span class="text-gray-400"><i class="fab fa-whatsapp"></i></span>'; // Disabled icon if no phone number
                // END OF WHATSAPP
                ...
                tr.innerHTML = `
                    ...
                    <td class="p-2">
                        ${whatsappLink}
                        ${order.can_delete ? `<button onclick="deleteOrder(${order.id})" class="text-red-600 ml-2"><i class="fas fa-trash"></i></button>` : ''}
                    </td>
                `;
    ...
    // ADDED TODAY 30 LOCATION FUNCTIONALITIES
    function initializeAutocomplete() {
        // Listing Location Autocomplete
        const listingLocationInput = document.getElementById('listingLocationInput');
        if (listingLocationInput) {
            const listingAutocomplete = new google.maps.places.Autocomplete(listingLocationInput, {
                types: ['geocode'],
                componentRestrictions: { country: 'ke' } // Restrict to Kenya
            });
            listingAutocomplete.addListener('place_changed', () => {
                const place = listingAutocomplete.getPlace();
                if (place.geometry) {
                    ...
                    listingLocationInput.value = place.formatted_address;
                }
            });
        }

        // Order Location Autocomplete
        const orderLocationInput = document.getElementById('orderLocationInput');
        if (orderLocationInput) {
            const orderAutocomplete = new google.maps.places.Autocomplete(orderLocationInput, {
                types: ['geocode'],
                componentRestrictions: { country: 'ke' } // Restrict to Kenya
            });
            orderAutocomplete.addListener('place_changed', () => {
                const place = orderAutocomplete.getPlace();
                if (place.geometry) {
                    document.getElementById('orderLatitude').value = place.geometry.location.lat();
                    document.getElementById('orderLongitude').value = place.geometry.location.lng();
                    orderLocationInput.value = place.formatted_address;
                }
            });
        }

        // 1ST
        // Farmer Location Autocomplete
        const farmerLocationInput = document.getElementById('farmerLocation');
        if (farmerLocationInput) {
            const farmerAutocomplete = new google.maps.places.Autocomplete(farmerLocationInput, {
                types: ['geocode'],
                componentRestrictions: { country: 'ke' }
            });
            farmerAutocomplete.addListener('place_changed', () => {
                const place = farmerAutocomplete.getPlace();
                if (place.geometry) {
                    document.getElementById('farmerLatitude').value = place.geometry.location.lat();
                    document.getElementById('farmerLongitude').value = place.geometry.location.lng();
                    farmerLocationInput.value = place.formatted_address;
                }
            });
        }
    }
    ...
    function useCurrentLocationListing() {
            ...
            navigator.geolocation.getCurrentPosition(
                (position) => {
                    ...
                    fetch(`https://maps.googleapis.com/maps/api/geocode/json?latlng=${lat},${lng}&key={{ google_maps_api_key }}&`)
                        ...
                        .then(data => {
                            if (data.results[0]) {
                                document.getElementById('listingLocationInput').value = data.results[0].formatted_address;
                            }
                        })
    ...
    // Use Current Location for Order
    function useCurrentLocationOrder() {
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(
                (position) => {
                    const lat = position.coords.latitude;
                    const lng = position.coords.longitude;
                    document.getElementById('orderLatitude').value = lat;
                    document.getElementById('orderLongitude').value = lng;
                    ...
                    fetch(`https://maps.googleapis.com/maps/api/geocode/json?latlng=${lat},${lng}&key={{ google_maps_api_key }}&`)
                        ...
                        .then(data => {
                            if (data.results[0]) {
                                document.getElementById('orderLocationInput').value = data.results[0].formatted_address;
                            }
                        })
                        .catch(error => console.error('Error reverse geocoding:', error));
                },
     ...
     // Use Current Location for Farmer
    function useCurrentLocationFarmer() {
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(
                (position) => {
                    const lat = position.coords.latitude;
                    const lng = position.coords.longitude;
                    document.getElementById('farmerLatitude').value = lat;
                    document.getElementById('farmerLongitude').value = lng;
                    ...
                    fetch(`https://maps.googleapis.com/maps/api/geocode/json?latlng=${lat},${lng}&key={{ google_maps_api_key }}`)
                        ...
                        .then(data => {
                            if (data.results[0]) {
                                document.getElementById('farmerLocation').value = data.results[0].formatted_address;
                            }
                        })
                        .catch(error => console.error('Error reverse geocoding:', error));
                },
    ...
    // ADDED TODAY 30 LOCATION FUNCTIONALITIES END
    ...
    document.addEventListener("DOMContentLoaded", function () {
           ...
           {% if user.userprofile.role == 'farmer' %}
            ...
            // LOCATION BASED EVENT LISTENERS
            initializeAutocomplete();
            const useCurrentLocationListingBtn = document.getElementById('useCurrentLocationListing');
            if (useCurrentLocationListingBtn) {
                useCurrentLocationListingBtn.addEventListener('click', useCurrentLocationListing);
            }
            const useCurrentLocationOrderBtn = document.getElementById('useCurrentLocationOrder');
            if (useCurrentLocationOrderBtn) {
                useCurrentLocationOrderBtn.addEventListener('click', useCurrentLocationOrder);
            }
   ...
   // Category Sidebar Toggle and Filter Logic
    ...
    // Initialize Google Maps for route display
    ...
    function initializeRouteMap(listingId) {
        ...
        mapInstances[listingId] = { map, directionsService, directionsRenderer };
    }
            ...
            if (data.status === 'success') {
                data.routes.forEach((route, index) => {
                    ...
                    tab.addEventListener('click', () => {
                        ...
                        document.getElementById(`map-${route.listing_id}`).classList.remove('hidden');
                        // Update details
                    ...
                    const [lat, lng] = route.origin.split(',').map(parseFloat);
                    const origin = { lat, lng };

                    mapInstances[route.listing_id].directionsService.route({
                        origin: origin,
                        destination: origin,
                        waypoints: waypoints,
                        ...
                        if (status === google.maps.DirectionsStatus.OK) {
                            mapInstances[route.listing_id].directionsRenderer.setDirections(result);
                        } else {
                            console.error(`Directions request failed for listing ${route.listing_name}: ${status}`);
                        }
                    });
    ...
    function saveFarmerLocation() {
        const latitude = document.getElementById('farmerLatitude').value;
        const longitude = document.getElementById('farmerLongitude').value;
        if (!latitude || !longitude) {
            alert('Please select a valid location using the address field or current location button.');
            return;
        }
        fetch('/market/update_farmer_location/', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRFToken': getCsrfToken()
    ...
    // Update DOMContentLoaded to initialize map and button
    document.addEventListener('DOMContentLoaded', function () {
        // Existing code...

        
        // Initialize route map
        // initializeRouteMap(); COMMMENTED OUT FOR NOW
